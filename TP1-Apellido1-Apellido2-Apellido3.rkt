...
; Estado inicial
(define START (make-st 0 (* HEIGHT .5) (* HEIGHT .5) 0.5 3 0))
...
(define (draw s)
  (place-image BALL
               (st-ball-x s) (st-ball-y s) BACKGROUND))
...
(define (bounce-x s)
  (cond
      [(hit-bar? s)        (reflect-ball-x s 1)]
      [(hit-right-wall? s) (reflect-ball-x s 1)]
      [else                s]
  )
)
...
(define (hit-top-wall? s)
  (if (< (st-ball-y s) (/ BALL-RADIUS 2)) #t #f))
